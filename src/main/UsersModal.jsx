...
const UsersModal = ({ open, onClose }) => {
  ...
  const getTokenInfo = (user) => {
    const raw = user.attributes?.notificationTokens;
              ...
              <Table>
                ...
                <TableBody>
                  {isLoading ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={7} align="center" style={{ padding: '40px', color: colors.error, lineHeight: 0.8 }}>
                        {t('sharedError')}: {error.message}
                      </TableCell>
                    </TableRow>
                  ) : paginatedUsers.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={7} align="center" style={{ padding: '40px', color: colors.textSecondary, lineHeight: 0.8 }}>
                        {t('sharedNoData')}
                      </TableCell>
                    </TableRow>
                  ) : (
                    <>
                      {paginatedUsers.map((user, index) => (
                        ...
                        >
                          ...
                          <TableCell>
                            {(() => {
                              const { count } = getTokenInfo(user);
                              return count > 0 ? (
                                <Tooltip title={`${count} dispositivo${count !== 1 ? 's' : ''} com push ativo`} arrow>
                                  <Chip
                                    icon={<NotificationsActiveIcon style={{ fontSize: '14px' }} />}
                                    label={`${count} token${count !== 1 ? 's' : ''}`}
                                    size="small"
                                    style={{
                                ...
                                <Tooltip title="Nenhum token registrado — app não instalado ou nunca aberto" arrow>
                                  <Chip
                                    icon={<NotificationsOffIcon style={{ fontSize: '14px' }} />}
                                    label="Sem push"
                                    size="small"
                                    style={{
