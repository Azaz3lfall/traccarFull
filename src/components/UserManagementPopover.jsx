...
/**
 * UserManagementPopover - Reusable user management UI (list, create, edit, delete, connections).
 * Renders as a popover/drawer with the platform's standard aesthetics.
 *
 * @param {Object} props
 * @param {boolean} props.desktop - Layout mode (desktop vs mobile)
 * @param {boolean} props.isMenuExpanded - Whether the main menu is expanded (affects positioning)
 * @param {boolean} props.isVisible - Controls visibility
 * @param {Function} props.onClose - Called when the popover should close
 * @param {string|number|null} [props.userId] - Optional. When set, opens the edit drawer for this user directly
 * @param {boolean} [props.showServerSettings=false] - When true, shows Server Settings action and drawer (for main menu context)
 * @param {string} [props.mode='full'] - 'full' = list + create/edit; 'createOnly' = only create drawer (for embedding in client form)
 * @param {Function} [props.onUserCreated] - Called after creating a user (when mode='createOnly' or from create flow)
 * @param {boolean} [props.closeOnCreate=false] - If true, close popover after creating user (for createOnly mode)
 *
 * @example
 * // From MainPage / menu (with Server Settings):
 * <FloatingUsersPopover desktop={desktop} isMenuExpanded={isMenuExpanded} isVisible={showUsersPopover} onClose={...} userId={editingUserId} />
 *
 * @example
 * // From OS Dashboard, Clientes, or other screens (without Server Settings):
 * const [showUserPopover, setShowUserPopover] = useState(false);
 * const [editingUserId, setEditingUserId] = useState(null);
 * <UserManagementPopover
 *   desktop={desktop}
 *   isMenuExpanded={isMenuExpanded}
 *   isVisible={showUserPopover}
 *   onClose={() => { setShowUserPopover(false); setEditingUserId(null); }}
...
const UserManagementPopover = ({ 
  ...
  showServerSettings = false, // When true, include Server Settings Drawer and related UI
  mode = 'full', // 'full' | 'createOnly' - createOnly shows only the create drawer
  onUserCreated = null, // Callback when user is created (for createOnly / client form integration)
  closeOnCreate = false, // If true, close after creating user
}) => {
  const isCreateOnly = mode === 'createOnly';
  ...
  // Auto-open create drawer when mode is createOnly
  useEffect(() => {
    if (isCreateOnly && isVisible) {
      setEditingUser({
        name: '',
        email: '',
        password: '',
  ...
  const handleCloseEditDialog = () => {
    ...
    // If we were editing a specific user (not from the list) or in createOnly mode, close the entire popover
  ...
  return (
    <AnimatePresence mode="wait">
      {isVisible && (
        <motion.div
          key="user-management-popover"
          initial={isCreateOnly ? { opacity: 0 } : { x: -400, opacity: 0 }}
          animate={isCreateOnly ? { opacity: 1 } : { x: 0, opacity: 1 }}
          exit={isCreateOnly ? { opacity: 0 } : { x: -400, opacity: 0 }}
          transition={{ duration: 0.2, ease: "easeOut" }}
        style={{
          position: 'fixed',
          top: isCreateOnly ? 0 : (!desktop ? '0px' : '8px'),
          left: isCreateOnly ? 0 : (!desktop ? '0px' : (isMenuExpanded ? '200px' : '63px')),
          width: isCreateOnly ? '100vw' : (!desktop ? '100vw' : `calc(100vw - ${isMenuExpanded ? '200px' : '63px'} - 10px)`),
          height: isCreateOnly ? '100vh' : (!desktop ? '100vh' : 'calc(100vh - 16px)'),
          zIndex: isCreateOnly ? 150000 : 9999,
          pointerEvents: 'auto',
          transition: 'left 0.3s ease'
        }}
      >
        <div style={{
          backgroundColor: isCreateOnly ? 'transparent' : colors.surface,
          border: isCreateOnly ? 'none' : `1px solid ${colors.border}`,
          borderRadius: isCreateOnly ? 0 : (!desktop ? '0px' : '0px 16px 16px 0px'),
          height: '100%',
          overflow: 'hidden',
          boxShadow: isCreateOnly ? 'none' : (!desktop ? 'none' : '0 2px 8px rgba(0, 0, 0, 0.1)'),
          display: 'flex',
          flexDirection: 'column',
        }}>
          {/* Header - hidden in createOnly mode */}
          {!isCreateOnly && (
          <>
          {/* Header */}
          <div style={{
          ...
          {/* Server Settings Drawer - only when showServerSettings is true */}
          {showServerSettings && (
          <AnimatePresence>
            {serverDialog && (
              <>
